...
public:
    vector<int> intersect(vector<int>& nums1, vector<int>& nums2) {
        // Determine which array has a shorter length
        vector<int>& shorter = (nums1.size() < nums2.size()) ? nums1 : nums2;
        vector<int>& longer = (nums1.size() < nums2.size()) ? nums2 : nums1;

        // Record each digit and its count in the map
        unordered_map<int, int> digitCount;
        for (int num : shorter) {
            digitCount[num]++;
        }

        // Iterate over the larger array and check for intersection
        vector<int> result;
        for (int num : longer) {
